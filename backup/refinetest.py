...
def refine():
	...
	refinewindow.geometry('1020x500')

	global img_refine

	global init_brightness
	...
	img_refine = img_refine.resize((x, y))														
	img_base = img_refine

	def std_rubikify(filename):

		filename = filename.convert('L')
		...
		filename = filename.convert('P', palette=Image.ADAPTIVE, colors=colour_quantity)
		...
		print('final palette......\n' + str(combo) + '\n')
		filename.putpalette(final)

		print('initial image out size - x ' + str(filename.size[0]) + ' / y ' + str(filename.size[1]) + '\n')
		...
		x, y, xoff, yoff = imresize(filename, 400)
		filename = filename.resize((x, y))														#resize the modified image for display in ui
		img_out1 = ptk.PhotoImage(filename)
	...
	def update():
		enhancer = ImageEnhance.Brightness(img_refine)
		img_d1h = enhancer.enhance(init_brightness - bri)
		img_d2h = enhancer.enhance(init_brightness - (bri*2))
		img_u1h = enhancer.enhance(init_brightness + bri)
		img_u2h = enhancer.enhance(init_brightness + (bri*2))
		img_baseh = enhancer.enhance(init_brightness)

		std_rubikify(img_d1h)
		std_rubikify(img_d2h)
		std_rubikify(img_u1h)
		std_rubikify(img_u2h)
		std_rubikify(img_baseh)

		img_d1 = ptk.PhotoImage(img_d1h)
		img_d2 = ptk.PhotoImage(img_d2h)
		img_u1 = ptk.PhotoImage(img_u1h)
		img_u2 = ptk.PhotoImage(img_u2h)
		img_base = ptk.PhotoImage(img_baseh)

		#d1.configure(image=d1)
		#d2.configure(image=d2)
		#u1.configure(image=u1)
		#u2.configure(image=u2)
		...
		u1.image = img_u1
		u2.image = img_u2
		base.image = img_baseb
	
		base_button = tk.Button(refinewindow, image=img_base, command=funcbase, borderwidth=1).place(x=410, y=10)
		d1 = tk.Button(refinewindow, image=img_d1, command=funcd1, borderwidth=1).place(x=210, y=10)
		d2 = tk.Button(refinewindow, image=img_d2, command=funcd2, borderwidth=1).place(x=10, y=10)
		u1 = tk.Button(refinewindow, image=img_u1, command=funcu1, borderwidth=1).place(x=610, y=10)
		u2 = tk.Button(refinewindow, image=img_u2, command=funcu2, borderwidth=1).place(x=810, y=10)
		ok_button = tk.Button(refinewindow, text='ok', command=ok).place(x=500, y=250)
